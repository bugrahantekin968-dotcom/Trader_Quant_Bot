...
try:
    from backtest.scorer import score as _python_score, exhaustion_score
except Exception:
...
class BotConfig:
	...
	# ── Timeframes and deep historical depths
	timeframes:		list[str] = field(default_factory=lambda: ["1d", "4h", "1h", "15m"])
	candles_per_tf: dict	  = field(default_factory=lambda: {
		"1d":  1000,	  # ~2.7 yil (derin gunluk S/R; backtest derinligiyle hizali)
		"4h":  2190,	  # ~1 year (365*6)
		"1h":  400,
		"15m": 300,
	})
	...
	history_check_interval:	 int = 3600	   # Closed-position scan once per hour
	...
	# Default: UTC 09:00–21:00 = Türkiye 12:00–24:00 — covers London open
	# killzone (07–11 UTC), NY killzone (12–16 UTC), and NY closure (16–21).
	# Set both to 0 and 24 to disable the time guard entirely.
	trading_hours_start_utc: int = 0
	trading_hours_end_utc:   int = 23
	...
	# Hard caps
	max_position_usd:		   float = 30_000.0	  # Per-symbol position ceiling (30% of $100k)
	max_total_exposure_usd:	   float = 60_000.0	  # Aggregate portfolio ceiling

	# Risk-based sizing (replaces score tiers): notional = risk_$ / SL-distance
	risk_pct_per_trade:		   float = 0.005    # risk 0.5% of balance per trade
	max_risk_usd:			   float = 1_000.0  # hard cap: max $1000 (1% of $100k) per trade
...
────────────────────────────────────────────────────────────────────
    ...
    this lookup table. NOTE the split treatment of conflicting HTF cases —
    they are NOT range/mixed, they are PRIME pullback / relief-rally setups.

        1D BULLISH & 4H BULLISH      → Long +25, Short  0
        1D BULLISH & 4H RANGING      → Long +18, Short +3
        1D BULLISH & 4H BEARISH      → Long +20, Short +5   ← PULLBACK IN UPTREND (prime LONG)
        1D & 4H both RANGING         → Long  +8, Short +8
        1D BEARISH & 4H BULLISH      → Long  +5, Short +20  ← RELIEF RALLY (prime SHORT)
        1D BEARISH & 4H RANGING      → Long  +3, Short +18
...
────────────────────────────────────────────────────────────────────
    ...
    C.1 Major S/R Proximity — Max 15 pts
        Compute distance % from current_price to nearest major support (below)
        and nearest major resistance (above). Apply ASYMMETRIC tiers — proximity
        to a level always favors the corresponding directional bias (long near
        support, short near resistance), even at wider distances appropriate
        for crypto volatility:

            At/inside Major SUPPORT   (within 0.3%)  → Long +15, Short  0
            Within 0.8% of Major SUPPORT             → Long +12, Short  0
            Within 1.5% of Major SUPPORT             → Long  +9, Short +1
            Within 2.5% of Major SUPPORT             → Long  +5, Short +2
            Middle of nowhere (>2.5% from both)      → Long  0, Short  0
            Within 2.5% of Major RESISTANCE          → Long +2, Short  +5
            Within 1.5% of Major RESISTANCE          → Long +1, Short  +9
            Within 0.8% of Major RESISTANCE          → Long  0, Short +12
            At/inside Major RESISTANCE (within 0.3%) → Long  0, Short +15

        If BOTH a support AND resistance are within 2.5%, classify by the
        CLOSER one. If equidistant, use "Middle of nowhere."

    C.2 S/R Quality Bonus — Max 5 pts
        GATE: This sub-score is 0 if C.1 was "Middle of nowhere".
              Otherwise, score ONLY the side that received C.1 points.

        Look at the nearest major level's metadata (label, touch_count). Apply
        the SINGLE highest applicable (no stacking):
...
3. Determine decision:
       long_total  >= 58  AND  long_total  > short_total  → "LONG"
       short_total >= 58  AND  short_total > long_total   → "SHORT"
       otherwise                                          → "WAIT"
...
4. SL / TP construction (only when decision is LONG or SHORT). Follow these
   ...
   STEP 4a — Structural SL with 1% buffer beyond the nearest major level:
       LONG  → structural_sl = nearest major support     × 0.99
       SHORT → structural_sl = nearest major resistance  × 1.01
   ...
   STEP 4d — Apply the maximum-distance ceiling (4.0%):
       If sl_pct > 0.04 → override decision to "WAIT" and stop here.
...
5. Position size (only when decision is LONG or SHORT):
   ...
   Recommend LONG/SHORT when your assessed score is >= 58 AND the setup is valid;
   below 58 -> "WAIT". (`score` = long_total for LONG, short_total for SHORT.)
...
6. The `entry_price` MUST NOT equal the exact current market price (MT5 would
   ...
   current_price (so the 1.5% floor and 4% ceiling apply correctly).
...
PY_GATE = 58   # Python on-filtre esigi = LLM karar esigi (58, backtest hizali). Python
...
def build_llm_prompt(payload: dict, lessons_path: str = "lessons_learned.json") -> str:
	...
	return (
		...
		+ "(1% structural buffer, 1.5% floor, 4.0% ceiling, RR ≥ 2.0).\n"
...
class AlgoBot:
	...
	async def _query_llm(self, payload: dict, data_sym: str) -> dict:
		...
		response = await self._ai_client.messages.create(
			...
			system=_SYSTEM_PROMPT.replace("__TP_CAP__", f"{_tp_cap_for(data_sym):.1f}"),
	...
	async def _check_closed_positions(self) -> None:
		...
		for entry in to_check:
			...
			changed = True
			logger.info(
				"[HISTORY] ticket=%s %s | close=%.4f | PnL=$%.2f",
	...
	async def _manage_active_positions(self, cg_manager: CoinglassManager) -> None:
		...
		for pos in our_positions:
			...
			st.setdefault("be_done", st.get("stage_50_done", False) or progress >= 50.0)

			crossed = min(4, int(progress // 25))

			# %50 → SL breakeven
			if not st["be_done"] and progress >= 50.0:
	...
	async def _run_symbol(
		...
		# 7b. PYTHON ON-FILTRE (maliyet gecidi) — once deterministik skor.
		#      Skor gecidin ALTINDAYSA -> WAIT ve LLM ATLANIR (~%99 dongu boyle).
		#      LLM cagrildiginda prompt/karar AYNEN eski gibi -> alinan islemler ayni.
		#      FAIL-SAFE: Python skorlama patlarsa sessizce eski LLM yoluna duser.
		if _python_score is not None:
			try:
				...
				logger.info("[%s] Python on-skor: L=%d S=%d (LLM esigi=%d gecit=%d)",
				            data_sym, _pl, _ps, self.cfg.min_score, PY_GATE)
				logger.info("[%s]   L-detay: %s", data_sym, _py["long_breakdown"])
				logger.info("[%s]   S-detay: %s", data_sym, _py["short_breakdown"])
				if _pl < PY_GATE and _ps < PY_GATE:
					logger.info("[%s] WAIT (Python on-filtre — LLM atlandi) | L=%d S=%d",
					            data_sym, _pl, _ps)
					return
			except Exception as _exc:
		...
		if side == "WAIT":
			...
			return
		# -- VETO (alt-TF donus): LLM short dese bile dusus trendi tukeniyorsa
		#    (exhaustion>=20) short ACMA. Deterministik; sadece short engeller,
		#    asla islem eklemez -- backtest short_exh_veto=20 ile AYNI mantik.
		if side == "SHORT" and exhaustion_score is not None:
			try:
				_exhv = float(exhaustion_score(tf_analysis, "short").get("total", 0.0))
				if _exhv >= 20.0:
					logger.info("[%s] VETO (alt-TF donus): short exhaustion=%.0f >= 20 -> islem ALINMADI", data_sym, _exhv)
					return
			except Exception:
				pass
