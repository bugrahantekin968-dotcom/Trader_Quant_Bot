...
⚠️  ENDPOINTS NEED LIVE VERIFICATION. Written to the same interface as the
    ...
    Kraken's servers, so the exact WS topics, REST paths and JSON field
    names must be checked against current Kraken API docs before production.
...
class KrakenClient:
    ...
    def __init__(self, coins: list[str]):
        self.coins = coins
        ep = config.EXCHANGE_ENDPOINTS.get("kraken", {})
        self.ws_url   = ep.get("ws", "")
        self.rest_url = ep.get("rest", "")

    async def stream(self, on_trade, on_liq) -> None:
        # VERIFY: subscription message shape + topic names per Kraken docs.
        if not self.ws_url:
            logger.warning("kraken: no ws endpoint configured; stream disabled")
            return
                    ...
                    async with session.ws_connect(self.ws_url, heartbeat=20) as ws:
                        ...
                        async for msg in ws:
                            if msg.type != aiohttp.WSMsgType.TEXT:
                                continue
                            # VERIFY: parse trades + liquidations from Kraken payload,
    ...
    async def fetch_context(self, session, coin):
        # VERIFY: Kraken ticker / open-interest / funding / long-short endpoints.
        # Must return MarketContext(exchange, coin, open_interest_usd, funding_rate,
        # long_ratio, mark_price, ts_ms) or None.
        logger.debug("kraken: fetch_context stub for %s — verify endpoints", coin)
        return None

    async def fetch_klines(self, session, coin, tf_label):
        # VERIFY: Kraken kline endpoint + row layout → list[Kline].
        return []
